...
class CrmLead(models.Model):
    ...
    def post_facebook_leads(self):
        ...
        crm_fb_app_id = params.get_param('crm_fb_app_id',
                                                 default=False)

        crm_fb_app_secret = params.get_param('crm_fb_app_secret',
                                         default=False)

        token_api="https://graph.facebook.com/oauth/access_token?client_id={}&client_secret={}&grant_type=client_credentials".format(
            crm_fb_app_id,
            crm_fb_app_secret
        )
        token=requests.get(token_api).json()
        ...
        current_date=datetime.today()-timedelta(days=2)
        date_int=int(datetime.timestamp(current_date))
        fn="ahmed"
        print(hashlib.sha256(fn.encode()).hexdigest())


        r = requests.post(fb_api,params={'access_token':event_token
            ,'data':
    json.dumps([
        {"action_source": "website", "event_id": 12345, "event_name": "Purchase", "event_time": date_int,
          "user_data": {
              "client_user_agent": "odoo",
              "em": "f660ab912ec121d1b1e928a0bb4bc61b15f5ad44d5efdc4e1c92a25e99b8e44a","fn":hashlib.sha256(fn.encode()).hexdigest(),"ln":hashlib.sha256(fn.encode()).hexdigest()}, "custom_data": {
                "currency": "USD",
                "value": "142.52"
            }}
