...
class Company(models.Model):
    ...
    show_bg_image = fields.Boolean(
        string='Add Login Background Image', readonly=False)
    ...
    enable_pwa_backend = fields.Boolean(string='Enable PWA', readonly=False)
    app_name_pwa_backend = fields.Char(
        'App Name', readonly=False, default='Spiffy')
    short_name_pwa_backend = fields.Char(
        'Short Name', readonly=False, default='Spiffy')
    description_pwa_backend = fields.Char(
        'App Description', readonly=False, default='Spiffy')
    image_192_pwa_backend = fields.Binary(
        'Image 192px', readonly=False, store=True)
    image_512_pwa_backend = fields.Binary(
        'Image 512px', readonly=False, store=True)
    start_url_pwa_backend = fields.Char(
        'App Start Url', readonly=False, default='/web')
    background_color_pwa_backend = fields.Char(
        'Background Color', readonly=False, default='#0097a7')
    theme_color_pwa_backend = fields.Char(
        'Theme Color', readonly=False, default='#0097a7')
    pwa_backend_shortcuts_ids = fields.Many2many(
        'pwa.shortcuts', string='PWA Shortcuts')
    spiffy_toobar_color = fields.Char(
        'Toolbar Color', readonly=False, default='#0097a7')
