...
class Survey_app(models.Model):
    ...
    def get_ratio(self):
        for rec in self:
            rec.ratio = (int(rec.priority_reception) + int(rec.priority_medical_procedure) + int(
                rec.priority_sterilization_hygiene)) / 15 * 100
...
class Patient(models.Model):
    ...
    def get_amount_totals(self):
        for record in self:
            ...
            total_net_not_completed = sum(list(
                record.teeth_treatment_ids.filtered(lambda t: t.state not in ['completed', 'invoiced']).mapped(
                    "net_amount")))
            ...
            for payment in obj_payment:
                if payment.payment_type == 'inbound':
                    ...
                    total_payment += payment.amount_egp
...
class Teeth(models.Model):
    ...
    def set_completion_date(self):
        for line in self.search([('invc_id', '!=', False)]):
            line.invc_id.completion_date = line.completion_date
...
class NewModule(models.Model):
    ...
    teeth_id = fields.Many2one(comodel_name="medical.patient", string="",
                               required=False, )
    completion_date = fields.Datetime(string="completion_date", required=False, )
