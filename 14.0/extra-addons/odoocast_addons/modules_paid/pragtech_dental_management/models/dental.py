...
class MedicalPatient(models.Model):
    ...
    def get_first_dignoses_date(self):
        for rec in self:
            first_dignoses_date = self.env['medical.teeth.treatment'].search(
                [('state', '=', 'completed'), ('patient_id', '=', rec.id)], order="completion_date",
                limit=1).completion_date
        ...
        for each in treatment_lines:
            ...
            if all_treatment:
                for each_trt in all_treatment:
                    ...
                    completion_date = each.get('completion_date', False)
                    if completion_date:
                    ...
                    try:
                        ...
                        if not op_id.completion_date and vals.get('completion_date'):
                            vals['completion_date'] = fields.Datetime.now()
                        treatment_id = op_id.write(vals)
                    except:
                        treatment_id = medical_teeth_treatment_obj.create(
                            vals)
                        if treatment_id.completion_date:
                            treatment_id.completion_date -= timedelta(hours=2)
    ...
    def create_birthday_scheduler(self):
        ...
        birthday_alert_id = self.env['patient.birthday.alert'].search([]).unlink()
...
class MedicalAppointment(models.Model):
    ...
    payment_id = fields.Many2one(comodel_name="account.payment", string="",
                                 required=False, )
    payment_ids = fields.One2many(comodel_name="account.payment", inverse_name="appointment_id", string="",
                                  required=False, )
...
class MedicalTeethTreatment(models.Model):
    ...
    def service_confirmation(self):
        for line in self:
            ...
            if line.state == 'completed' and not line.invc_id:
                ...
                inv_values = {
                    ...
                    'teeth_id': line.patient_id.id or False,
                }
    ...
    def onchange_completion_date(self):
        if self.completion_date and self.completion_date.date() != fields.Date.today():
            if self.env.user.id not in self.env.ref("pragtech_dental_management.group_dental_mng_menu").users.ids:
                raise ValidationError(_("The Completion Date should be today only"))

    def service_confirmation_server_action(self):
        for line in self:
            # if line.state == 'completed':
            if line.state == 'completed' and not line.invc_id.state != 'cancel':
                journal_id = self.env['account.journal'].search([
                    ('type', '=', 'sale')], limit=1)
                inv_line_main = {
                    'name': line.description.name,
                ...
                inv_values = {
                    ...
                    'dentist': line.dentist.id,
                    'move_type': 'out_invoice',
                    'invoice_date': line.completion_date or datetime.now().strftime(DF) or False,
                    'journal_id': journal_id and journal_id.id or False,
                    'teeth_id': line.patient_id.id or False,
                }
                acc_id = self.env['account.move'].sudo().create(inv_values)
