        ...
        for scheduler in self:
            if scheduler.schedule_date < fields.Datetime.now():
                raise ValidationError(_(
                    'Please select a date equal/or greater than '
                    'the current date.'))

    def set_schedule_date(self):
        start_activity_ids = self.utm_campaign_id.mailing_activities_ids. \
            filtered(lambda m: m.trigger == 'campaign_start')
        ...
        for activity in start_activity_ids:
            ...
            self.utm_campaign_id.schedule_date = self.schedule_date
            # self.utm_campaign_id.set_children_activity_states(
            #      activity, schedule=self.schedule_date)
